...
let check_board_property name_x ?size (prop : board -> bool) =
  assert_equal true
    ~msg:(Printf.sprintf "QCheck test %s" name_x)
    QCheck.(run (mk_board_test ~name:name_x (arbitrary_board ?size) prop))

let check_full_board_property name_x ?size (prop : board -> bool) =
  assert_equal true
    ~msg:(Printf.sprintf "QCheck test %s" name_x)
    QCheck.(run (mk_board_test ~name:name_x (arbitrary_full_board ?size) prop))
