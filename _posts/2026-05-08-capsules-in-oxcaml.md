...
enough? OxCaml's answer is the **capsule** — a way to bundle state
with its lock so the lock discipline becomes a type-checker job rather
than a programmer convention.
...
"always lock before access" is a programmer convention — the
compiler can't tell what mutex you meant for what state, and an
unlocked `Hashtbl.add` is a runtime data race that nothing catches.
...
`gensym` closure — which captures both `mutex` and `counter` — type-
checks at portable mode and is safe to send to another domain.
...
**`@ uncontended`** — we hold the lock; no other domain can be
touching the data. The next two lines, `incr c; !c`, are ordinary
