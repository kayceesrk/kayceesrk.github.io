...
<script async
  ...
  src-load="{{ '/assets/mirage-crypto.js' | absolute_url }}"
></script>
...
webpage. Let's use `mirage-crypto`.
...
```bash
$ x-ocaml --effects mirage-crypto -o mirage-crypto.js
```

This produces the JavaScript artifact that can be used in the webpage. It may be
instructive to look at the [source]() of this post to see how the compiler and
the libraries are integrated into this blog post.
